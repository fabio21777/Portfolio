...
export type PortfolioType = typeof portifolioFabio;

export const getInfoPortifolio = (userName?: string | string[]): PortfolioType => {
  // Se um nome de usuário foi fornecido, salva no localStorage apenas no cliente
  if (userName && import.meta.client) {
    localStorage.setItem('userName', String(userName));
  }

  // Se nenhum nome de usuário foi fornecido ou é inválido
  if (!userName && !userNameGlobal) {
    // Tenta recuperar do localStorage (apenas cliente)
